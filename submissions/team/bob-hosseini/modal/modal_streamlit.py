...
# ---
# ## WattWise Energy Consumption Forecasting App
#
# Streamlit application for forecasting building energy consumption using SARIMAX model.
...
def main():
    ...
    try:
        header_image = Image.open("/root/image.jpg")
    ...
    @st.cache_data
    def load_data():
        with open("/root/data/data_cleaned.pkl", 'rb') as f:
    ...
    df_data, df_24h_complete, df_data_history = load_data()
    ...
    @st.cache_data
    def load_model_data():
        with open("/root/data/data_split.pkl", 'rb') as f:
    ...
    train_hr, exog_train, exog_test = load_model_data()

    # load model checkpoint
    @st.cache_data
    def load_model_checkpoint():
        with open("/root/models/sarimax_checkpoint.json", 'r') as f:
            checkpoint = json.load(f)
        return (tuple(checkpoint["order"]), 
                tuple(checkpoint["seasonal_order"]), 
                checkpoint["params"])

    order, seasonal_order, params = load_model_checkpoint()
    ...
    @st.cache_data
    def load_feature_relevance_profile():
        with open("/root/data/feature_relevance_profile.pkl", 'rb') as f:
    ...
    feature_names, feature_coefficients = load_feature_relevance_profile()
